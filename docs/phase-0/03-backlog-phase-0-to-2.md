...
- **P0-09 — Model-turn choreography and full-trace sensitivity v0.1 drafted; dependency, review, and acceptance pending:** The [desk-authored review draft](evidence/P0-09-model-turn-choreography-and-sensitivity-v0.1.md) specifies the correlated proposal/one-repair/commit/tool-random-child/apply/narrate flow, deterministic fallback, cache/stream policy, provider-neutral review contract, current-source rate example, cost/latency sensitivity, and metadata-only telemetry. It is `NOT IMPLEMENTED`, uses no executed model trace, selects no provider, and does not finalize provisional gates. P0-07 remains unaccepted, so this draft cannot satisfy P0-09's dependency or done evidence.
- **P0-10 — Data-flow and threat-model v0.1 drafted; dependency, review, and acceptance pending:** The [desk-authored review draft](evidence/P0-10-data-flow-and-threat-model-v0.1.md) maps local authority, safety/data controls, provider/credential alternatives, central metadata, OS residue, reports, exports, deletion, threats, controls, incident response, and adversarial fixtures. It is `NOT IMPLEMENTED`, proves no mitigation, selects no provider/runtime, and records `P010-BLK-01`: no static provider secret may ship in a local client. P0-07 remains unaccepted, so this draft cannot satisfy P0-10's dependency or done evidence.
...
- **Current desk-only critical path:** Advisory-review and reconcile P0-07 v0.1 without broadening its authority, then reconcile the provisional P0-09 and P0-10 consumers to the accepted result. Phase 0 P0-07/P0-09/P0-10 acceptance remains blocked on the P0-06/P0-05 evidence chain and required named-human/role reviews; provider/qualified diligence and executed P1B evidence remain later integration gates. P0-11 and later tasks have not started. Human research is deferred: recruitment and fieldwork remain NO-GO, G0 remains closed, and no Phase 1 production task is authorized.
...
| [P0-09](evidence/P0-09-model-turn-choreography-and-sensitivity-v0.1.md) | Specify end-to-end model turn choreography and produce model/data/vendor plus full-trace cost/latency sensitivity | A8 + A2 | P0-01, P0-03, P0-07 | Every pre-roll call, tool round trip, post-result narration, retry, cache transfer, stream point, and fallback is budgeted; candidate class—not vendor commitment—reviewed |
| [P0-10](evidence/P0-10-data-flow-and-threat-model-v0.1.md) | Produce a visibility- and safety-aware data-flow diagram and threat model | A3 | P0-01, P0-03, P0-07 | Trust boundaries, retention/deletion, secrets, authorization, prompt injection, content boundaries, stop path, denial-of-wallet, and mitigations reviewed by A6/A7 |
